...
def bleu_metric(prediction, reference, xlingual=False):
...
def compute_metrics(predictions, references, xlingual=False):
    ...
    for pred, gold in zip(predictions, references):
        ...
        bleu += metric_max_over_ground_truths(
            bleu_metric, prediction=pred, ground_truths=gold, xlingual=xlingual
        )
        
    em = 100.0 * em / len(references)
    print("length of references",len(references))
    rougeL = 100.0 * rougeL / len(references)
    bleu = bleu / len(references)
