...
def add_model_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--discrim-path', type=str, help='discrim path')
    group.add_argument('--discrim', type=str, help='discriminator')
...
def add_hp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--critic_it', type=int, help='iter number of the critic model')
    group.add_argument('--generator_it', type=int, help='iter number of the generator model', default=1)
    group.add_argument('--generator_scale', type=float, default=1.0, help='scale of the generator loss')
    group.add_argument('--critic_scale', type=float, default=1.0, help='scale of the discriminator loss')
    group.add_argument('--ds_scale', type=float, default=1.0, help='scale of the distillation loss')
...
def get_args():
    ...
    if args.type == "eval_main":
        ...
        args.save = save_path
    elif args.type == "rkld" or args.type == "fkld":
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
             f"-{args.teacher_ckpt_name}" + f"-{args.teacher_peft_name}" if args.teacher_peft_name is not None else ""),
            (f"{args.type}-e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}-NN{args.n_nodes}-kd{args.kd_ratio}") + \
            (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "") + \
            (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else "") + \
            args.save_additional_suffix
        )
        args.save = save_path
    elif args.type == "wgan_rkld" or args.type == "wgan_fkld":
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
             f"-{args.teacher_ckpt_name}" + f"-{args.teacher_peft_name}" if args.teacher_peft_name is not None else ""),
            (f"{args.type}-gs_{args.generator_scale}-ds_{args.ds_scale}-d_it-{args.critic_it}-g_it-{args.generator_it}-e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}-NN{args.n_nodes}-kd{args.kd_ratio}-crit_{args.discrim}") + \
            (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "") + \
            (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else "") + \
            args.save_additional_suffix
        )
        args.save = save_path
    elif args.type == "rgan_rkld" or args.type == "rgan_fkld":
        save_path = os.path.join(
            args.save,
            (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
             f"-{args.teacher_ckpt_name}" + f"-{args.teacher_peft_name}" if args.teacher_peft_name is not None else ""),
            (f"{args.type}-gs_{args.generator_scale}-ds_{args.ds_scale}-cs_{args.critic_scale}-d_it-{args.critic_it}-g_it-{args.generator_it}-e{args.epochs}-bs{args.batch_size}-lr{args.lr}-G{args.gradient_accumulation_steps}-N{args.n_gpu}-NN{args.n_nodes}-kd{args.kd_ratio}-crit_{args.discrim}") + \
            (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "") + \
            (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else "") + \
            args.save_additional_suffix
        )
