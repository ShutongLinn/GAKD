...
from discriminator import MLPEmbeddingLLMDiscriminator
...
def get_discriminator_model(args,device: int):
    discriminator = MLPEmbeddingLLMDiscriminator(args.model_path)
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if True:
                ...
                real_labels = torch.ones(batch_size, 1, device=device, dtype=torch.bfloat16)
                fake_labels = torch.zeros(batch_size, 1, device=device, dtype=torch.bfloat16)
                
                for _ in range(discriminator_iter):
                    d_real = discriminator_model(teacher_logits)
                    d_fake = discriminator_model(student_logits)
                    # print("shape",d_real.shape)
                    # exit()
            ...
            if True:
                ...
                real_labels = torch.ones(batch_size, 1, device=device, dtype=torch.bfloat16)
                d_fake = discriminator_model(logits)
                generator_loss = criterion(d_fake, real_labels)

                # print("loss",loss.item(),"generator loss",generator_loss.item())
                if (epoch > 0):
                    loss += generator_loss * args.generator_scale
...
def main():
    ...
    discriminator_model, discriminator_optimizer = get_discriminator_model(args,device)
